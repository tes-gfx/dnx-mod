...
static int dnx_gem_mmap(struct drm_gem_object *obj, struct vm_area_struct *vma)
{
	struct dnx_bo *bo;

	dev_dbg(obj->dev->dev, "%s", __func__);
...
int dnx_gem_prime_vmap(struct drm_gem_object *obj, struct iosys_map *map)
{
	dev_dbg(obj->dev->dev, "%s", __func__);
...
static const struct drm_gem_object_funcs dnx_gem_object_funcs = {
	.free = dnx_gem_free_object,
	.vmap = dnx_gem_prime_vmap,
	.mmap = dnx_gem_mmap,
	.vm_ops = &vm_ops,
};
...
__dnx_bo_create(struct drm_device *drm, size_t size)
{
	...
	gem_obj->funcs = &dnx_gem_object_funcs;
